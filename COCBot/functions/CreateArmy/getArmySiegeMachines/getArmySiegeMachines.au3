...
Func getArmySiegeMachines($bOpenArmyWindow = False, $bCloseArmyWindow = False, $bCheckWindow = False, $bSetLog = True, $bNeedCapture = True)
	...
	If Ubound($aGetSiegeCap) = 2 Then
		If $bSetLog Then SetLog("اجمالي الة الحصار: " & $aGetSiegeCap[0] & "/" & $aGetSiegeCap[1])
	...
	If UBound($aCurrentSiegeMachines, 1) >= 1 Then
		For $i = 0 To UBound($aCurrentSiegeMachines, 1) - 1 ; Loop through found Troops
			...
			$sSiegeName = $g_aiCurrentSiegeMachines[$iSiegeIndex] >= 2 ? $g_asSiegeMachineNames[$iSiegeIndex] & " " : $g_asSiegeMachineNames[$iSiegeIndex] & ""
			...
			If $g_bDebugSetlogTrain Then Setlog($sSiegeName & " فتحة (" & Slot($aSiegeCoords[0], $aSiegeCoords[1]) & "," & 196 & ")")

			If $bSetLog Then SetLog(" - " & $g_aiCurrentSiegeMachines[$iSiegeIndex] & " " & $sSiegeName & " متاح", $COLOR_SUCCESS)
		Next
	EndIf
