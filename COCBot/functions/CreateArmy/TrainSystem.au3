...
Func TrainSystem()
	...
	If $g_abDonateOnly[$g_iCurAccount] And ProfileSwitchAccountEnabled() And $g_bIsFullArmywithHeroesAndSpells Then
		SetLog("تبرع الوضع فقط ويتم إعداد الجيش الخاص بك!", $COLOR_DEBUG) ;Debug
		If $g_bFirstStart Then $g_bFirstStart = False
		DoubleTrain($g_bQuickTrainEnable)
		Return
	Endif
	...
	ElseIf $g_bIsFullArmywithHeroesAndSpells Or ($g_CurrentCampUtilization = 0 And $g_bFirstStart) Then
		...
		If ($g_CurrentCampUtilization = 0 And $g_bFirstStart) Then
			...
			SetLog(" - انتقل الى نافذة الجيش و حدد موقع التدريب السريع!", $COLOR_ACTION1)
		EndIf
...
Func TestMaxCamp()
	...
	If UBound($ArmyCamp) = 3 Then
		...
		Else
			...
			SetLog(" - الجيش هو: " & $ArmyCamp[0], $COLOR_ACTION)
			$ToReturn = 0
		EndIf
	EndIf
...
Func TrainCustomArmy()
	...
	If $g_bDebugSetlogTrain Then SetLog(" == التدريب المخصص == ", $COLOR_ACTION)
	...
	If $g_bDoubleTrainDone And $g_bDoubleTrain Then
		If $g_bDebugSetlogTrain Then SetLog("تم عمل التدريب المذدوج دعنا نتخطى التدريب")
		TrainSiege()
		If _Sleep(250) Then Return
		EndGainCost("Train")
		ClickP($aAway, 2, 0, "#0346") ;Click Away
...
Func CheckIfArmyIsReady()
	...
	If $g_bDebugSetlogTrain Then
		Setlog("الأبطال جاهزون: " & String($bFullArmyHero))
		Setlog("الملوك متاحين Num: " & $g_iHeroAvailable) ;  	$eHeroNone = 0, $eHeroKing = 1, $eHeroQueen = 2, $eHeroWarden = 4
		Setlog("بحث البطل انتظر تمكين [$DB] Num: " & $g_aiSearchHeroWaitEnable[$DB]) ; 	what you are waiting for : 1 is King , 3 is King + Queen , etc etc
		Setlog("بحث البطل انتظر تمكين [$LB] Num: " & $g_aiSearchHeroWaitEnable[$LB])
		Setlog("القرية المهجورة: " & BitAND($g_aiSearchHeroWaitEnable[$DB], $g_iHeroAvailable))
		Setlog("القرية النشطة: " & BitAND($g_aiSearchHeroWaitEnable[$LB], $g_iHeroAvailable))
		Setlog("أنت 'لا' في انتظار الأبطال: " & String($g_aiSearchHeroWaitEnable[$DB] = $eHeroNone And $g_aiSearchHeroWaitEnable[$LB] = $eHeroNone))
		Setlog("هو الانتظار لأبطال نشط : " & IsWaitforHeroesActive())
	EndIf
	...
	If Not IsWaitforHeroesActive() And Not $g_bDropTrophyUseHeroes And Not $bFullArmyHero Then
		If $g_iHeroAvailable > 0 Or Number($g_aiCurrentLoot[$eLootTrophy]) <= Number($g_iDropTrophyMax) Then
			$bFullArmyHero = True
		Else
			SetLog("في الانتظار الملوك ل اسقاط الكؤؤس!", $COLOR_ACTION)
		EndIf
	EndIf
	...
	If $g_bIsFullArmywithHeroesAndSpells Then
		...
		SetLog("رئيس ، هل جيشك جاهز؟ نعم إنه كذلك!", $COLOR_SUCCESS)
	Else
		SetLog("رئيس ، هل جيشك جاهز؟ لا ليس بعد!", $COLOR_ACTION)
		If $sLogText <> "" Then SetLog(@TAB & "انتظر ل " , $COLOR_ACTION)
	EndIf
			...
			For $i = 0 To (UBound($rWTT) - 1)
				...
				If $rWTT[$i][1] > 0 Then ; If Count to Train Was Higher Than ZERO
					...
					If $NeededSpace <= $LeftSpace Then ; If Needed Space was Equal Or Lower Than Left Space
						...
						Local $sTroopName = ($rWTT[$i][1] > 1 ? $g_asTroopNamesPlurall[$iTroopIndex] : $g_asTroopNames[$iTroopIndex])
						...
						Else
							SetLog("لاتوجد موراد كافية للتدريب " & $rWTT[$i][1] & "  " & $sTroopName, $COLOR_ACTION)
							$g_bOutOfElixir = True
						EndIf
					Else ; If Needed Space was Higher Than Left Space
						...
						Local $sTroopName = ($CountToTrain > 1 ? $g_asTroopNamesPlurall[$iTroopIndex] : $g_asTroopNames[$iTroopIndex])
						...
						Else
							SetLog("لاتوجد مورارد كافية للتدريب" & $CountToTrain & "  " & $sTroopName, $COLOR_ACTION)
							$g_bOutOfElixir = True
						EndIf
					EndIf
				EndIf
				...
				If $rWTT[$i][1] > 0 Then ; If Count to Train Was Higher Than ZERO
					...
					Else ; If Needed Space was Higher Than Left Space
						...
						If CheckValuesCost($rWTT[$i][0], $CountToTrain) Then
							...
							TrainIt($iTroopIndex, $CountToTrain, $g_iTrainClickDelay)
						Else
							SetLog("لاتوجد موارد كافية للتدريب " & $CountToTrain & "  " & $sTroopName, $COLOR_ACTION)
							$g_bOutOfElixir = True
						EndIf
					EndIf
				EndIf
...
Func BrewUsingWhatToTrain($Spell, $Quantity) ; it's job is a bit different with 'TrainUsingWhatToTrain' Function, It's being called by TrainusingWhatToTrain Func
	...
	If $Quantity = 9999 Then
		SetLog("صنع " & $sSpellName & " Spell Cancelled " & @CRLF & _
				"                  السبب: كفى كما هو محدد في واجهة المستخدم الرسومية " & @CRLF & _
				"                               هذا التعويذ لا تستخدم في الهجوم")
		Return True
	EndIf
	If Not $g_bRunState Then Return
	...
	Select
		Case $g_bIsFullArmywithHeroesAndSpells = False
			...
			If $NeededSpace <= $LeftSpace Then ; If Needed Space was Equal Or Lower Than Left Space
				...
				Else
					SetLog("لاتوجد موارد كافية لصنع التعويذة " & $Quantity & "  " & $sSpellName & ($Quantity > 1 ? " Spells" : " Spell"), $COLOR_ACTION)
					$g_bOutOfElixir = True
				EndIf
		...
		Case $g_bIsFullArmywithHeroesAndSpells = True
			...
			If $NeededSpace <= $LeftSpace Then ; If Needed Space was Equal Or Lower Than Left Space
				If CheckValuesCost($Spell, $Quantity) Then
					...
					TrainIt($iSpellIndex, $Quantity, $g_iTrainClickDelay)
				Else
					SetLog("لاتوجد موارد كافية لصنع التعويذات " & $Quantity & "  " & $sSpellName & ($Quantity > 1 ? " Spells" : " Spell"), $COLOR_ACTION)
					$g_bOutOfElixir = True
				EndIf
			EndIf
	EndSelect
...
Func DragIfNeeded($Troop)
	...
	If IsDarkTroop($Troop) Then
		...
		If $g_bDebugSetlogTrain Then SetLog("قوات الظلام: " & $bCheckPixel)
		For $i = 1 To 3
	...
	Else
		...
		If $g_bDebugSetlogTrain Then SetLog("قوات ارضية: " & $bCheckPixel)
		For $i = 1 To 3
			If Not $bCheckPixel Then
		...
		Next
	EndIf
	SetLog("فشل في التحقق من القوات " & $g_asTroopNames[TroopIndexLookup($Troop, "DragIfNeeded")] & " Position or Failed to Drag Successfully", $COLOR_ERROR)
	Return False
...
Func RemoveExtraTroops($toRemove)
	...
	If UBound($toRemove) > 0 Then ; If needed to remove troops
		...
		If Not IsQueueEmpty("Troops") Then
			...
			For $i = 0 To (UBound($toRemove) - 1)
				...
				If IsAlreadyTraining($toRemove[$i][0]) Then
					SetLog($g_asTroopNames[TroopIndexLookup($toRemove[$i][0])] & " خطا في الجيش الثاني المزدوج!", $COLOR_INFO)
					DeleteQueued("Troops")
					$iResult = 2
				EndIf
			Next
			...
			If TotalSpellsToBrewInGUI() > 0 Then
				...
				For $i = $CounterToRemove To (UBound($toRemove) - 1)
					...
					If IsAlreadyTraining($toRemove[$i][0], True) Then
						SetLog($g_asSpellNames[TroopIndexLookup($toRemove[$i][0]) - $eLSpell] & " خطا في الجيش الثاني المزدوج!", $COLOR_INFO)
						DeleteQueued("Spells")
						$iResult = 2
					EndIf
				Next
		...
		SetLog("حزف القوات: ", $COLOR_INFO)
		$CounterToRemove = 0
		; Loop through Troops needed to get removed Just to write some Logs
		For $i = 0 To (UBound($toRemove) - 1)
			If IsSpellToBrew($toRemove[$i][0]) Then ExitLoop
			$CounterToRemove += 1
			SetLog(" - " &  $g_asTroopNamesPlurall[TroopIndexLookup($toRemove[$i][0])] & ": " & $toRemove[$i][1] & " ", $COLOR_SUCCESS)
		Next

		If TotalSpellsToBrewInGUI() > 0 Then
			If $CounterToRemove <= UBound($toRemove) Then
				SetLog("حزف التعويذات: ", $COLOR_INFO)
				For $i = $CounterToRemove To (UBound($toRemove) - 1)
					SetLog(" - " & $g_asSpellNames[TroopIndexLookup($toRemove[$i][0]) - $eLSpell] & ": " & $toRemove[$i][1] & " ", $COLOR_SUCCESS)
				Next
			EndIf
		EndIf
		...
		If Not _ColorCheck(_GetPixelColor(806, 516, True), Hex(0xCEEF76, 6), 25) Then ; If no 'Edit Army' Button found in army tab to edit troops
			SetLog("لا يمكن العثور على / التحقق من زر", $COLOR_WARNING)
		...
		If Not _ColorCheck(_GetPixelColor(806, 567, True), Hex(0xCEEF76, 6), 25) Then ; If no 'Okay' button found in army tab to save changes
			SetLog("لا يمكن العثور على / التحقق من الزر", $COLOR_WARNING)
		...
		If Not _ColorCheck(_GetPixelColor(508, 428, True), Hex(0xFFFFFF, 6), 30) Then ; If no 'Okay' button found to verify that we accept the changes
			SetLog("لا يمكن العثور على / التحقق من الز", $COLOR_WARNING)
			ClickP($aAway, 2, 0, "#0346") ;Click Away
			Return False ; Exit function
		EndIf
...
Func WhatToTrain($ReturnExtraTroopsOnly = False, $bSetLog = True)
	...
	If $g_bIsFullArmywithHeroesAndSpells And Not $ReturnExtraTroopsOnly Then
		If $g_iCommandStop = 3 Or $g_iCommandStop = 0 Then
			...
			Return $ToReturn
		EndIf
		SetLog(" - الجيش الخاص بك ممتلئ ، دعونا نجعل القوات قبل الهجوم!", $COLOR_INFO)
		; Elixir Troops
		For $i = 0 To $eTroopCount - 1
...
Func CheckQueueTroops($bGetQuantity = True, $bSetLog = True, $x = 839)
	Local $aResult[1] = [""]
	If $bSetLog Then SetLog("التحقق من الجيش الثاني...", $COLOR_INFO)
...
Func CheckQueueSpells($bGetQuantity = True, $bSetLog = True, $x = 835)
	...
	;$hTimer = TimerInit()
	If $bSetLog Then SetLog("التحقق من التعويذات الاضافية...", $COLOR_INFO)
...
Func SearchArmy($sImageDir = "", $x = 0, $y = 0, $x1 = 0, $y1 = 0, $sArmyType = "", $bSkipReceivedTroopsCheck = False)
	...
	For $iCount = 0 To 10
		...
		If Not getReceivedTroops(162, 200, $bSkipReceivedTroopsCheck) Then
			...
			ExitLoop
		Else
			If $iCount = 1 Then SetLog("لقد تلقيت جنود القلعة! انتظر 5 في...")
...
Func DeleteQueued($sArmyTypeQueued, $iOffsetQueued = 802)
	...
	While Not IsQueueEmpty($sArmyTypeQueued, True, False)
		If $x = 0 Then SetLog(" - حذف " & $sArmyTypeQueued & " Queued!", $COLOR_INFO)
		If _Sleep(20) Then Return
		If Not $g_bRunState Then Return
...
Func MakingDonatedTroops($sType = "All")
	...
	SetLog("  تدريب القوات للدعم", $COLOR_ACTION1)
	If $areThereDonTroop > 0 Then
		...
		For $i = 0 To UBound($avDefaultTroopGroup, 1) - 1
			...
			If $avDefaultTroopGroup[$i][4] > 0 Then
				...
				If $avDefaultTroopGroup[$i][2] * $avDefaultTroopGroup[$i][4] <= $RemainTrainSpace[2] Then ; Troopheight x donate troop qty <= avaible train space
					...
					SetLog(" - مدربة " & $avDefaultTroopGroup[$i][4] & " " & $sTroopName, $COLOR_ACTION)
					$avDefaultTroopGroup[$i][4] = 0
		...
		If $sType = "All" Then
			...
			If $RemainTrainSpace[0] < $RemainTrainSpace[1] Then ; army camps full
				...
				SetLog(" - مدربة " & $howMuch & " archer(s)!", $COLOR_ACTION)
				If _Sleep(1000) Then Return ; Needed Delay, OCR was not picking up Troop Changes
			EndIf
		EndIf
	...
	If $areThereDonSpell > 0 Then
		...
		For $i = 0 To $eSpellCount - 1
			...
			If $g_aiDonateSpells[$i] > 0 Then
				...
				;PureClick($pos[0], $pos[1], $howMuch, 500)
				If _Sleep($DELAYRESPOND) Then Return ; add 5ms delay to catch TrainIt errors, and force return to back to main loop
				SetLog(" - مصنوعة " & $howMuch & " " & $g_asSpellNames[$i] & ($howMuch > 1 ? " Spells" : " Spell"), $COLOR_ACTION)
	...
	If $areThereDonSiegeMachine > 0 Then
		...
		For $iSiegeIndex = $eSiegeWallWrecker To $eSiegeMachineCount - 1
			...
			If $g_aiDonateSiegeMachines[$iSiegeIndex] > 0 Then
				...
				If _CheckPixel($checkPixel, True, Default, $g_asSiegeMachineNames[$iSiegeIndex]) Then
					...
					SetLog(" - مدربة " & $HowMany & " " & $g_asSiegeMachineNames[$iSiegeIndex] & ($HowMany > 1 ? " SiegeMachines" : " SiegeMachine"), $COLOR_ACTION)
		...
		SetLog("إجمالي ورشة عمل الحصار: " & $aGetSiegeCap[0] & "/" & $aGetSiegeCap[1])
...
Func CheckIsEmptyQueuedAndNotFullArmy()
	...
	SetLog(" - التحقق:لا يوجد جيش ثاني و الجيش غير كامل", $COLOR_ACTION1)
...
Func TestTrainRevamp2()
	...
	$g_bDebugOcr = True
	SetLog("ابدأ ...... نافذة OpenArmy .....")

	Local $timer = __TimerInit()

	getArmyTroops(False, False, False, False)

	SetLog("وقت القوات: " & Round(__TimerDiff($timer) / 1000, 2) & "'s")

	SetLog("نهاية ...... نافذة OpenArmy .....")
	$g_bDebugOcr = False
	$g_bRunState = False
...
Func CheckValuesCost($Troop = "Arch", $troopQuantity = 1, $DebugLogs = 0)
	...
	If $g_bDebugSetlogTrain Or $DebugLogs Then
		...
		SetLog(" - اكسير: " & _NumberFormat($nElixirCurrent) & " / دارك اكسير: " & _NumberFormat($nDarkCurrent), $COLOR_INFO)
