...
Func getArmySpells($bOpenArmyWindow = False, $bCloseArmyWindow = False, $bCheckWindow = False, $bSetLog = True, $bNeedCapture = True)
	...
	If UBound($aCurrentSpells, 1) >= 1 Then
		For $i = 0 To UBound($aCurrentSpells, 1) - 1 ; Loop through found Spells
			...
			$sSpellName = $g_aiCurrentSpells[$iSpellIndex] >= 2 ? $g_asSpellNames[$iSpellIndex] & "  " : $g_asSpellNames[$iSpellIndex] & "  " ; Select the right Spell Name, If more than one then use Spells at the end
	...
	For $index = 0 To UBound($aCurrentSpellsLog) - 1
		If $aCurrentSpellsLog[$index][1] > 0 And $bSetLog Then SetLog(" - " & $aCurrentSpellsLog[$index][1] & " " & $aCurrentSpellsLog[$index][0] & "  ", $COLOR_SUCCESS)
	Next
