...
Func getArmyCCSiegeMachines($bOpenArmyWindow = False, $bCloseArmyWindow = False, $bCheckWindow = False, $bSetLog = True, $bNeedCapture = True)
	...
	If $bSetLog And Ubound($aGetSiegeCap) = 2 Then
		SetLog("مجموعة الات الحصار في قلعة القبيلة: " & $aGetSiegeCap[0] & "/" & $aGetSiegeCap[1])
	...
	If UBound($aCurrentCCSiegeMachines, 1) >= 1 Then
		For $i = 0 To UBound($aCurrentCCSiegeMachines, 1) - 1 ; Loop through found Troops
			...
			$sCCSiegeName = $g_aiCurrentCCSiegeMachines[$iCCSiegeIndex] >= 2 ? $g_asSiegeMachineNames[$iCCSiegeIndex] & " " : $g_asSiegeMachineNames[$iCCSiegeIndex] & ""
			...
			If $g_bDebugSetlogTrain Then Setlog($sCCSiegeName & " فتحة (" & 650 & "," & 498 & ")")

			If $bSetLog Then SetLog(" - " & $g_aiCurrentCCSiegeMachines[$iCCSiegeIndex] & " " & $sCCSiegeName & " متاح", $COLOR_SUCCESS)
		Next
	EndIf
