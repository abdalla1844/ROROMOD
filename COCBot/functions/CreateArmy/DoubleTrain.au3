...
Func DoubleTrain($bQuickTrain = False)
	...
	If $bDebug then SetLog($bQuickTrain ? " التدريب المذوج  ", $COLOR_ACTION)
	StartGainCost()
	...
	While 1
		...
		If $bSetlog Then SetLog("التحقق من نافذة القوات : " & $TroopCamp[0] & "/" & $TroopCamp[1] * 2)
		If $TroopCamp[1] = 0 Then ExitLoop
		If $bSetlog And $TroopCamp[1] <> $g_iTotalCampSpace Then _
			SetLog("مجموعة غير صحيحة من القوات: " & $g_iTotalCampSpace & " vs Total camp: " & $TroopCamp[1] & @CRLF & "                 التدريب المذدوج لا يعمل بشكل جيد", $COLOR_DEBUG)
	...
	If $TotalSpellsToBrewInGUI = 0 Then
		If $bDebug Then SetLog("لا يوجد سبيل مهيئ تخطي نافذة التعويذات", $COLOR_DEBUG)
	...
	Else
		...
		While 1
			...
			If $bSetlog Then SetLog("التحقق من نافذة التعويذات: " & $SpellCamp[0] & "/" & $SpellCamp[1] * 2)
			...
			If $SpellCamp[1] <> $TotalSpellsToBrewInGUI Or $SpellCamp[1] <> $g_iTotalSpellValue Then
				If $bSetlog And Not $g_bForceBrewSpells Then SetLog("غير صحيح التحرير والسرد: " & $TotalSpellsToBrewInGUI & "/" & $g_iTotalSpellValue & _
																	" مقابل مجموع المعسكر: " & $SpellCamp[1] & @CRLF & "                 التدريب المذدوج لا يعمل بشكل جيد", $COLOR_DEBUG)
...
Func TrainFullQueue($bSpellOnly = False, $bSetlog = True)
	...
	If $bSetlog Then SetLog("التحقق " & ($bSpellOnly ? "نافذة التعويذات: " : "نافذة الجيش: ") & $CampOCR[0] & "/" & $CampOCR[1] * 2)
...
Func DoubleQuickTrain($bSetlog, $bDebug)
	...
	While 1
		Local $TroopCamp = GetCurrentArmy(48, 160)
		If $bSetlog Then SetLog("التحقق من نافذة الجيش: " & $TroopCamp[0] & "/" & $TroopCamp[1] * 2)
...
;~ 	If Not IsArray($ArmyCamp) Then $ArmyCamp = GetCurrentArmy(48, 160)
	...
	Local $iTotalQueue = 0
	If $bDebug Then SetLog("التحقق من الجيش الثاني المذدوج: " & $ArmyCamp[0] & "/" & $ArmyCamp[1] * 2, $COLOR_DEBUG)
	...
	If $ArmyCamp[0] < $ArmyCamp[1] * 2 Then
		; Train remain
		SetLog("التحقق من الجيش المذدوج:")
		...
		For $i = 0 To UBound($aiQueueTroops) - 1
			...
			If $g_aiArmyCompTroops[$iIndex] - $aiQueueTroops[$iIndex] > 0 Then
				...
				SetLog("    مفقود: " & $g_asTroopNames[$iIndex] & "  " & $rWTT[UBound($rWTT) - 1][1])
		...
		SetLog("التحقق من نافذة الجيش: " & $ArmyCamp[0] & "/" & $ArmyCamp[1] * 2 & ($ArmyCamp[0] < $ArmyCamp[1] * 2 ? ". Top-up queue failed!" : ""))
...
;~ 	If Not IsArray($ArmyCamp) Then $ArmyCamp = GetCurrentArmy(43, 160)
	...
	Local $iTotalQueue = 0
	If $bDebug Then SetLog("التحقق من التعويذات المذدوجة: " & $ArmyCamp[0] & "/" & $ArmyCamp[1] * 2, $COLOR_DEBUG)
	...
	For $i = 0 To UBound($aiQueueSpells) - 1
		If $aiQueueSpells[$i] - $g_aiArmyCompSpells[$i] > 0 Then
			SetLog("هناك خطا في تعويذات التدريب المذوج")
			Return False
		EndIf
	Next
	If $ArmyCamp[0] < $ArmyCamp[1] * 2 Then
		; Train remain
		SetLog("التحقق من التعويذات المذدوجة:")
		Local $rWTT[1][2] = [["Arch", 0]] ; what to train
		For $i = 0 To UBound($aiQueueSpells) - 1
			...
			If $g_aiArmyCompSpells[$iIndex] - $aiQueueSpells[$iIndex] > 0 Then
				...
				SetLog("    مفقود: " & $g_asSpellNames[$iIndex] & "  " & $rWTT[UBound($rWTT) - 1][1])
				ReDim $rWTT[UBound($rWTT) + 1][2]
			EndIf
		Next
		Local $bIsFullArmywithHeroesAndSpells = $g_bIsFullArmywithHeroesAndSpells
		...
		SetLog("التحقق من نافذة السبيلات: " & $NewSpellCamp[0] & "/" & $NewSpellCamp[1] * 2 & ($NewSpellCamp[0] < $ArmyCamp[1] * 2 ? ". Top-up queue failed!" : ""))
