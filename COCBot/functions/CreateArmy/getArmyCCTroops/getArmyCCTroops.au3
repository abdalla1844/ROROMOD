...
Func getArmyCCTroops($bOpenArmyWindow = False, $bCloseArmyWindow = False, $bCheckWindow = False, $bSetLog = True, $bNeedCapture = True, $bGetSlot = False)
	...
	If UBound($aCurrentCCTroops, 1) >= 1 Then
		For $i = 0 To UBound($aCurrentCCTroops, 1) - 1 ; Loop through found CC Troops
			...
			$sTroopName = $g_aiCurrentCCTroops[$iTroopIndex] >= 2 ? $g_asTroopNamesPlurall[$iTroopIndex] : $g_asTroopNames[$iTroopIndex] ; Select the right Troop Name, If more than one then use the Plural
			If $bSetLog Then SetLog(" - " & $g_aiCurrentCCTroops[$iTroopIndex] & "  " & $sTroopName & " (Clan Castle)", $COLOR_SUCCESS) ; Log What Troop is available and How many
		Next
	EndIf
